...
def test_run_xhs_login_qrcode_falls_back_to_http_api_when_mcp_qrcode_errors(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    raw = base64.b64decode(
    ...
    output_path = tmp_path / "xhs-login-error-fallback.png"
    monkeypatch.setattr(
        "ptsm.application.use_cases.xhs_login.fetch_xhs_login_qrcode_via_api",
        lambda server_url: {
            "timeout": "4m0s",
        ...
        publisher=FakePreflightPublisher(
            {
                ...
                "server_url": "http://localhost:18061/mcp",
                "login_status": "❌ 未登录",
                "qrcode_error": "Failed to fetch XiaoHongShu login QR code: TimeoutError",
            }
        ),
    )
    ...
    assert result["qrcode"]["output_path"] == str(output_path)
    assert result["qrcode"]["timeout"] == "4m0s"
    assert result["qrcode_error"] == "Failed to fetch XiaoHongShu login QR code: TimeoutError"
