...
class PlanRunner:
    ...
    def run(
        ...
        *,
        ...
        persisted_state = (
            _load_state(normalized_state_path)
            if resume and normalized_state_path is not None
            else None
        )
        ...
        if dry_run:
            result = PlanRunResult(
                status=(
                    str(persisted_state.get("status", "dry-run"))
                ...
                dry_run=True,
                state_path=(
                    str(normalized_state_path) if normalized_state_path is not None else None
                ),
        ...
        for task in tasks:
            ...
            for attempt in range(start_attempt, effective_max_attempts + 1):
                ...
                if codex_result.exit_code != 0:
                    ...
                    self._persist_state(
                        normalized_state_path=normalized_state_path,
                        plan_path=normalized_plan_path,
                        status=(
                            "failed" if attempt == effective_max_attempts else "in_progress"
                        ),
                        verify_commands=normalized_verify_commands,
                        max_attempts=max_attempts,
                        tasks=tasks,
    ...
    def _persist_state(
        self,
        *,
        normalized_state_path: Path | None,
        plan_path: str,
        status: str,
        verify_commands: list[str],
        max_attempts: int,
...
def build_default_state_path(plan_path: Path, base_dir: Path | None = None) -> Path:
    state_dir = base_dir or (Path.cwd() / ".codex-plan-runner" / "runs")
...
def _build_codex_prompt(
    *,
    ...
    verify_lines = (
        ...
        or "- No verification commands configured"
    )
    failure_section = ""
    if failure_feedback:
        failure_section = (
    ...
    return (
        ...
        f"{task.prompt or 'Implement the task using the specification above.'}\n\n"
        f"{done_when_section}"
