...
6. For a psychology text carousel, require `image_generation.status=committed`, **one topic / one dynamic 1–18-page set**, ordered pages with `slide_id` / `page_sha256` / `file_sha256`, a matching immutable `manifest.json`, and a completed page-aware asset ledger; never score, relay, or publish a partial set. `carousel_delivery.status=ready` is an outer-relay handoff only, not proof that a chat/IM sender delivered it. Prefer one ordered multi-image sender call; otherwise require serial per-attachment ACK before the next call. Sender ACK/outcome/retry are relay-owned evidence, not PTSM experiment results.
...
Content that requires more than 18 pages/images is not an experiment batch parameter. Stop and ask for shortening or use the
same three-way router: `one_carousel` is the supported ordinary dynamic 1–18-page carousel; `multiple_posts` requires
separate user confirmation and an independent receipt for every post; `independent_assets` is unsupported because
8–12 **independent image assets** are outside this psychology path. Do not loop, split, repeat, fabricate a ready
receipt, or treat `max_text_units` as image count.
...
For current v3 learning, “complete committed carousel” includes a successful page-aware operational
