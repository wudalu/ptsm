...
6. For a psychology text carousel, require `image_generation.status=committed`, **one topic / one 4–7-page set**, ordered pages with `page_sha256` / `file_sha256`, a matching immutable `manifest.json`, and a completed page-aware asset ledger; never score, relay, or publish a partial set. `carousel_delivery.status=ready` is an outer-relay handoff only, not proof that a chat/IM sender delivered it. Sender ACK/outcome/retry are relay-owned evidence, not PTSM experiment results.
...
An explicit request for more than 7 pages/images (including 12) is not an experiment batch parameter. Stop at the
same three-way router: `one_carousel` is the supported ordinary 4–7-page carousel; `multiple_posts` requires
