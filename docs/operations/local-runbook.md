...
# JSON directions include `selection_policy`, `open_direction_id`, `direction_type`,
# and `scene_fit`. The public shape is 3 curated directions plus 1 PTSM-returned
# `open_scene`; when changing the scene, rerun `guide-post` instead of reusing
# the previous direction set. Publishing/login/browser commands are unchanged.
uv run python -m ptsm.bootstrap guide-post \
