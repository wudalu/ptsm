...
- `learning_series` 不新增一套泛心理学 skill。它只在 `modern_psychology_post` 已绑定 catalog contract 时收紧现有心理学 style：固定 series badge、概念、批准解释、微练习、适用范围和专业边界必须原样可核验，且读者可见 JSON 必须逐字段等于 catalog-derived 的 controlled lesson template，不能补造课程、来源、疗效、学习结果或“更有网感”的额外心理学断言。目录选择 policy 固定为 `catalog_learning_series`：无 lesson 的 guide 明确返回 `selection_required`，选中 lesson 后才给出该课独立的 title/cover hook 和 catalog-owned image plan。普通 `open_scene` guidance、runtime research、历史正文 memory、manual image override 都不可替代它。
