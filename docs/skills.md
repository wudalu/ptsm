...
- `reddit_discussion_scan` / `reddit_curation_style` / `xhs_reddit_curation_hashtagging` 只服务 `reddit_curation_daily_post`。其中 `reddit_discussion_scan` 读取已获批 Reddit app-only OAuth 的公开 hot/top 英文讨论，或在 app 创建受阻时用 `REDDIT_PUBLIC_JSON_FALLBACK=true` 和非占位 `REDDIT_USER_AGENT` 走低频只读 public JSON fallback；它优先筛选 AI 工具焦虑、心理/生活压力和工作流切口。缺少 `REDDIT_CLIENT_ID`、`REDDIT_CLIENT_SECRET`、`REDDIT_USER_AGENT` 且没有可用 fallback 时会注入缺配置/缺权限上下文。`reddit_curation_style` 要求“Reddit 英文讨论 -> 翻成中文语境 -> 国内读者共鸣 -> 可收藏小结 -> 评论区问题”，并禁止长段原文搬运、展示用户名、伪亲历、心理诊断/治愈承诺和投资建议；`xhs_reddit_curation_hashtagging` 要求 `#Reddit` 等搜索友好标签。
