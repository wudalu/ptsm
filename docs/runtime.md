...
- `guide-post` 是应用层只读选题引导，不启动 workflow、不创建 run、不发布、不调用 live XHS / topic-radar。它用 `ptsm.domain.topic_guidance` 的确定性 selector、`application/use_cases/topic_guidance_packs.py` 的非心理学本地 topic pack 和心理学专用 brief 数据，为当前九个 playbook 从大于展示数的本地候选池中返回 4 个场景相关方向：`modern_psychology_post`、`fengkuang_daily_post`、`human_enrichment_daily_post`、`sushi_poetry_daily_post`、`wuxia_character_post`、`ai_tech_daily_post`、`daily_english_post`、`world_cup_daily_post`、`reddit_curation_daily_post`。selector 只用用户 scene 命中 `scene_keywords`，只用 lane 名称命中 `lane_affinity`，再按 `diversity_key` 优先保留不同内容家族，避免 lane 文本把所有场景推到同一个高优先级方向。每个方向带 `scene_fit`、趋势信号、病毒式 hook、适合场景、内容角度、保存工具、评论提示和避坑。输出不包含内部 research 文档路径、原始来源说明、URL 或 provenance。
