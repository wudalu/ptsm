...
- `reddit_discussion_scan` 的 runtime context 服务 `reddit_curation_daily_post`，通过已获批的 Reddit app-only OAuth 读取公开英文讨论的 hot/top 列表，按 AI 工具焦虑、心理/生活压力和工作流相关性筛选适合中文读者的来源。缺少 Reddit 环境变量时会注入 `missing_credentials` 上下文，提示按 Reddit Responsible Builder Policy 取得 explicit approval 后配置 `REDDIT_CLIENT_ID`、`REDDIT_CLIENT_SECRET` 和 `REDDIT_USER_AGENT`，并要求生成内容不要声称来自最新 Reddit 讨论。
