...
这不是 12 张或任意页数的批量图片 API：显式要求超过 7 页（例如 12 张）时，wrapper/operator 必须先
澄清是要一组 4–7 页的普通帖，还是多个分别确认的帖子；不得静默拆分、循环运行、复用一组或承诺
未支持的数量。`max_text_units` 描述单页文字密度，不是图片/页数。
