...
- `modern_psychology_post` 的自动图片策略默认使用一组 4–7 张 `psychology_text_card_v1`：封面仍只放一个困境钩子和最多一条短支持句，inner cards 才承载有界的场景、轻机制、工具、范围/专业边界与评论入口。parent `image_plan` 保留既有 routing fields 并增加 `carousel_style` / `slides`；slide 顺序就是发布顺序。`guide-post` 把它公开为 `format_archetype=text_carousel`、page range、ordered roles 和 `command_hint=--auto-generate-image`，没有新的手动 pagination 或 batch 参数。仅完整 receipt + ledger 成功的 ordinary set 才可产生 `carousel_delivery.status=ready`，供外部 relay 严格按 `attachments[].order` 升序转发有序 `attachments`，并用每页 `page_sha256` / `file_sha256` 核验；不得按 path 或 filename 重新排序。PTSM 不负责该外部交付。显式 ordinary `--local-image-style` 仍可选择单张 `iphone_notes` / `note_card` / `wechat_chat`，但 learning-series 禁止 override。
