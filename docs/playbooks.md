...
- `modern_psychology_post` 专门输出现代心理困境观察内容，优先从关系、消息、睡前或职场的具体瞬间进入；标题不提前抛心理学术语或 `不是你...`，正文以场景、轻机制、自然保存动作/工具、角色/阵营/填空式评论入口和专业帮助边界组织。`psychology_safety` 继续约束诊断、治疗承诺、药物建议和危机处理边界；睡眠恢复/轻养生只是既有心理学子线实验，不是新领域或医疗养生结论。默认绑定 `acct-psychology-local`。
  ...
  普通心理学默认是一个主题的一组按内容需要动态生成的 1–18 张 `psychology_text_card_v1` semantic text carousel：低密度 `cover_hook` 后按需使用 `concrete_scene`、`light_mechanism`、`save_tool`、`scope_boundary` / `professional_boundary` 和 `comment_prompt`。同一次 draft 不预设页数、不按正文字符数切页，也不为每张图另找主题；`max_text_units` is per-page text density, not image count。ordinary 的 recent 12 successful complete carousel receipts 只约束同账号/同 playbook 的相同 inner cards，且 reservation 只在完整 receipt + ledger 后 commit；learning-series 不计入。

  若完整表达需要 >18 页，必须停止并要求缩短内容或走三路 router：`one_carousel` 是支持的一题一组动态 1–18 页；`multiple_posts` 只有用户逐帖确认、各自独立 guide/run/receipt 时才支持；`independent_assets` 是 unsupported，必须转交另行授权的素材流程。不得静默拆分、循环、重复或把第三路伪装为前两路。ordinary `carousel_delivery.status=ready` 只表示 local receipt handoff；relay ACK/outcome/retry 由外部 sender 负责，不能成为 PTSM 已送达或已发布结论。
...
- `modern_psychology_post` 的自动图片策略默认使用一组动态 1–18 张 `psychology_text_card_v1`：封面仍只放一个困境钩子和最多一条短支持句，inner cards 才承载有界的场景、轻机制、工具、范围/专业边界与评论入口。parent `image_plan` 保留既有 routing fields 并增加 `carousel_style` / `slides`；slide 顺序就是发布顺序，renderer 在图上绘制 `NN / TT` 页码。`guide-post` 把它公开为 `format_archetype=text_carousel`、dynamic page range、ordered roles 和 `command_hint=--auto-generate-image`，没有新的手动 pagination 或 batch 参数。仅完整 receipt + ledger 成功的 ordinary set 才可产生 `carousel_delivery.status=ready`；外部 relay 优先以一个保序多图消息发送，否则严格按 `attachments[].order` 串行发送且等待上一张 ACK，并用每页 `slide_id` / `page_sha256` / `file_sha256` 核验；不得并发或按 path/filename 重排。PTSM 不负责该外部交付。显式 ordinary `--local-image-style` 仍可选择单张 `iphone_notes` / `note_card` / `wechat_chat`，但 learning-series 禁止 override。
- 超过 18 页的心理学内容必须先缩短，或由 wrapper/operator 要求用户逐帖确认 `multiple_posts`；不得自动拆帖。`carousel_delivery.status=ready` 仍只描述 immutable local receipt；relay 的 ACK/outcome/retry 是外部 sender 的独立 record，不能成为 PTSM 已送达或已发布结论。
