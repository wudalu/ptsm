...
- `reddit_curation_daily_post` 专门输出 Reddit英文讨论转译内容，从英文 Reddit hot/top 讨论里选 AI 热点、心理困境、效率工作流等适合中文读者的角度，要求来源边界、中文语境解释、可收藏小结和评论区方向选择。默认绑定 `acct-reddit-curation-local`。读取最新 Reddit 讨论需要配置 `REDDIT_CLIENT_ID`、`REDDIT_CLIENT_SECRET` 和 `REDDIT_USER_AGENT`；缺配置时 dry-run 仍可完成，但不能声称来自最新 Reddit 热帖。
