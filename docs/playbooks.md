...
- 九个小红书 playbook 都加载共享 `xhs_image_strategy`，使正文生成阶段可以同时给出图片后端和样式计划；这个策略与各领域 style / hashtag skill 并列，不替代领域内容约束。
...
- `reddit_curation_daily_post` 专门输出 Reddit英文讨论转译内容，从英文 Reddit hot/top 讨论里选 AI 热点、心理困境、效率工作流等适合中文读者的角度，要求来源边界、中文语境解释、可收藏小结和评论区方向选择。默认绑定 `acct-reddit-curation-local`。读取最新 Reddit 讨论需要按 Reddit Responsible Builder Policy 为该用途取得 explicit approval，并配置获批 app 的 `REDDIT_CLIENT_ID`、`REDDIT_CLIENT_SECRET` 和 `REDDIT_USER_AGENT`；缺配置时 dry-run 仍可完成，但不能声称来自最新 Reddit 热帖。
- 九个真实 XHS 内容质量 playbook 都有 playbook-local `evaluation.yaml`，声明 executor 内容质量 judge 为 `required`，并用确定性 node contract 检查领域标签、正文结构、评论提示、收藏触发和实验指令泄漏。
...
`reddit_curation_daily_post` 的 `evaluation.yaml` 要求 `#Reddit`、Reddit/英文讨论来源边界、中文解释、收藏触发和评论区问题，并禁止伪亲历、心理诊断/治愈承诺、投资建议和实验指令泄漏。
