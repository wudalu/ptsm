...
- `modern_psychology_post` 的 deterministic fallback 会按场景簇生成不同候选：周日/周一预焦虑、被说想太多后的边界压力、下班后被消息拉回工位、会议尴尬复盘、脑内复盘会、普通回复复盘接龙、睡前短视频/信息过载、孤独/比较焦虑。离线草稿标题不暴露心理机制或 `不是你` 句式，机制名只在场景铺开后轻量出现一次以内，并以自然保存动作和角色/阵营/填空式评论提示收束。
