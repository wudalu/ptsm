...
- `outputs/generated_images/*`（单图，以及 `<output-stem>-<set-id-prefix>/manifest.json` + ordered PNG 的 immutable psychology carousel set）
- `outputs/artifacts/generated-image-assets/assets.jsonl`
...
- completed `modern_psychology_post` learning-series artifact 额外保留 `psychology_learning_mode`、`psychology_learning_series_id`、`psychology_learning_curriculum_version`、`psychology_learning_lesson_id`、`psychology_learning_lesson_number`、opaque `psychology_learning_evidence_manifest` 和通过的 `psychology_learning_gate`。custom `user_confirmed` artifact 还必须有 `psychology_learning_catalog_receipt`：只含 `schema_version`、origin、controlled template version、catalog digest、approval id、proposal fingerprint 与 publication plan；builtin artifact 不写该 receipt。已确认的 controlled-template-v1 receipt 继续重建原单卡；historic builtin curriculum v1 与 historic custom template v2 保留旧版 carousel；historic builtin curriculum v2 保留 controlled template v3 的固定 7 页；current builtin curriculum v3 与新确认 revision 使用 controlled template v4 的温柔克制、动态 1–18 页、emoji-free catalog-owned carousel。它不保留自由 scene、raw proposal topic/outline goal、原始研究链接、作者、source、URL 或 local path；`psychology.learning_receipt` 可离线重建 catalog 并审计 the entire artifact，而不只检查 receipt。受控 artifact root 内若被 custom workflow 写入 raw provenance，应用层会标为 invalid，永不复用或发布；为避免同 UID 写者在检查与 unlink 间替换可变路径，运行时不会在线删除，文件留给 trusted offline maintenance。
- custom confirmed catalog 的 production-progress sidecar 以 series/version 绑定，保存 completed lesson IDs 和总数；在 custom `guide-post` 响应中它显示为 `series.production_progress`，其 `kind` 是 `operator_content_production`。首次 custom 存储由可信操作者在所有 writer 停止后通过 `provision-psychology-learning-storage` 创建/验证固定私有树，普通运行不会重建它。publication order 属于独立的 `series.publication_plan`，不是 progress sidecar。它只表示安全 artifact 已产出的运营内容，不是读者学习进度，也不表示已发布；未请求图片时，原有 safe content-artifact completion 可更新它；请求图片时，还必须先有完整 committed carousel 和验证通过的 strict receipt。任何 `psychology_carousel_generation_failed` 都不推进。builtin catalog 不返回这些 custom schedule/progress 字段。write path 保证并发 idempotent update 不丢课次。若 atomic rename 后的边界检查失败，响应为 `psychology_learning_progress_persist_failed`；这具有 at-least-once 语义，不能由该状态推断 sidecar 一定未写入。恢复可信存储后可幂等重试同一课次，不可信残留交给 trusted offline maintenance。
- current controlled template v4 learning 请求图片时，“完整 committed carousel”同时要求 page-aware operational asset ledger 成功；sealed artifact 不展示该 ledger，但 ledger failure 仍在 progress 之前 fail closed。
...
- relay acknowledgement/outcome/retry 不写入 PTSM observability。外部 relay 必须在自己的 durable record 中以
  ...
  delivered/partial/failed 结果；发送顺序必须严格遵循 `carousel_delivery.attachments[].order`，不得按 path
  或 filename 重排。对于 relay 自己编排的 `multiple_posts`，还可保存 `batch_id`、`target_count`、
...
- `xhs-record-metrics` 会把人工或只读收集到的小红书单帖表现追加到 `outputs/artifacts/xhs-post-metrics/metrics.jsonl`。记录会读取原始 run artifact，带上 `playbook_id`、`account_id`、`topic_selection.topic_direction_id`、标题、封面文案、图片样式、`image_count`、`carousel_style`、发布标识和 `2h` / `24h` / `72h` checkpoint，并计算 `interaction_score = likes + collects*2 + comments*4 + shares*6`、`interaction_rate` 和 `like_rate`。旧 artifact 与单张封面规范化为 `image_count=1`、空 carousel style；verified psychology carousel 使用真实动态 1–18 页数和 `psychology_text_card_v1`。
