...
- custom confirmed catalog 的 production-progress sidecar 以 series/version 绑定，保存 completed lesson IDs 和总数；在 custom `guide-post` 响应中它显示为 `series.production_progress`，其 `kind` 是 `operator_content_production`。首次 custom 存储由可信操作者在所有 writer 停止后通过 `provision-psychology-learning-storage` 创建/验证固定私有树，普通运行不会重建它。publication order 属于独立的 `series.publication_plan`，不是 progress sidecar。它只表示安全 artifact 已产出的运营内容，不是读者学习进度，也不表示已发布；completed dry-run 或内容成功后的 publish failure 可以更新它，preflight/workflow/eval/final-artifact failure 不可以。builtin catalog 不返回这些 custom schedule/progress 字段。write path 保证并发 idempotent update 不丢课次。若 atomic rename 后的边界检查失败，响应为 `psychology_learning_progress_persist_failed`；这具有 at-least-once 语义，不能由该状态推断 sidecar 一定未写入。恢复可信存储后可幂等重试同一课次，不可信残留交给 trusted offline maintenance。
