---
title: XHS Vertical Focus
status: active
owner: ptsm
last_verified: 2026-05-30
source_of_truth: false
related_paths:
  - docs/xhs-topics/index.md
  - docs/playbooks.md
  - docs/skills.md
  - docs/research/2026-05-23-xhs-viral-meme-product-hooks.md
  - docs/research/2026-05-30-xhs-domain-opportunity-and-workflow-review.md
---

# XHS Vertical Focus

## Selection Frame

这里只保留适合 PTSM 当前阶段的垂类，不做“大而全行业大全”。

筛选标准：

- 有明确的平台热度或增长证据
- 能连续产出，而不是只能蹭一次事件
- 能通过 `search_feeds -> get_feed_detail` 形成可重复分析链
- 能和现有或可预见的 playbook 语气对接

## 2026-05-30 Live Probe Update

2026-05-30 用本地 `xiaohongshu-mcp` 做了一轮只读搜索级探针，覆盖现有与候选领域关键词：`人类丰容`、`修复系手作`、`普通人用AI`、`轻养生`、`睡眠恢复`、`宠物户外`、`文博非遗`、`情绪疗愈`、`发疯文学`、`苏轼`、`武侠`、`每日英语`、`世界杯`。默认 `collect-xhs-patterns` 因 MCP 登录预检慢/500 失败；一次性探针用更长 timeout 直接调用 `search_feeds` 成功返回每个关键词前 5 条搜索结果。

这次结果不等于全站榜单，但给当前优先级一个清晰信号：

- `世界杯` 因小红书 2026-05-28 官宣成为 2026 美加墨世界杯持权转播商出现极强短期热度，适合现有 `world_cup_daily_post` 立刻做内容日历。
- `情绪疗愈`、`睡眠恢复`、`轻养生` 是最强的常青机会，其中睡眠/轻养生是最清晰的新领域候选。
- `每日英语`、`文博非遗`、`人类丰容`、`古诗词/苏轼` 都有较强保存/文化/分享信号，适合继续优化现有 playbook；诗词侧现在承接到更宽的 `classic_poetry_quote_post`。
- `武侠` 在这次搜索级样本中热度最低，应保留为深度小众域，不作为新增投入优先级。

完整证据、样本分数和 workflow 建议见 [`docs/research/2026-05-30-xhs-domain-opportunity-and-workflow-review.md`](../research/2026-05-30-xhs-domain-opportunity-and-workflow-review.md)。

## Recommended Focus Lanes

### 0. 人类丰容 / 零成本日常变量实验

当前信号：

- 2026-05-16 研究记录显示，`#人你该丰容了`、`#家的丰容计划`、零成本丰容、手作心流、Colorwalk、适我主义等方向在小红书生活方式内容里持续升温，详见 [`docs/research/2026-05-16-xhs-new-theme-research.md`](../research/2026-05-16-xhs-new-theme-research.md)。
- 2026-05-17 的 live MCP 采样计划已落成周期采集方式：定期用 `collect-xhs-patterns` 采集 `人类丰容`、`家的丰容计划`、`低成本改造`、`钩织`、`拼豆` 等关键词，再用 `analyze-xhs-patterns` 生成本地 pattern library，普通生成不实时抓高互动帖子。
- 2026-05-23 爆品梗调研把 `适我主义`、`新独居`、`手作心流`、`年味角落` 明确映射到现有 `human_enrichment_daily_post`，并要求每条内容落到一个角落、物件、路线、材料或十分钟动作，详见 [`docs/research/2026-05-23-xhs-viral-meme-product-hooks.md`](../research/2026-05-23-xhs-viral-meme-product-hooks.md)。
- 这个方向天然适合 3:4 竖版封面和轮播：封面、原本状态、小变量/材料平铺、三步清单、改变后细节、评论区提问。

为什么值得做：

- 它比泛「治愈系」更具体，能持续围绕一个角落、一个物件、一个路线或一个动作发帖。
- 它和当前 `发疯文学` 的情绪出口、`现代心理困境观察` 的低风险行动都有相邻关系，但不落入诊断或治疗承诺。
- 它对 PTSM 的图片生成和 artifact review 有明确推动：需要记录图片/轮播形式，而不只是正文质量。

适合的帖子角度：

- 工位/书桌/床头/玄关的低成本变量
- 一周日常变量实验
- Colorwalk 或感官散步
- 手作材料平铺、十分钟心流
- 「原本惯性 -> 一个变量 -> 三步清单 -> 评论区交作业」
- pattern hooks：`突然意识到...`、`人，你该...`、`before vs after`、`低成本变量清单`、`过程原来这么简单`

类型判断：`trend + evergreen + visual-first`

### 1. 修复系手作 / 情绪疗愈

当前信号：

- 千瓜在 2026 年 Q1 热词快报里把“修复自然，手搓万物”列为高热方向，相关话题浏览量超过 30 亿。<https://www.qian-gua.com/information/detail/3322>
- 同一批观察里，`手作钩织` 近 90 天笔记数增长 8400%+，话题浏览量 3.7 亿+。<https://www.qian-gua.com/information/detail/3318>

为什么值得做：

- 它同时覆盖“热点”和“常青内容”。
- 既能承接情绪价值，也能承接生活方式表达。
- 最适合把当前 `fengkuang_daily_post` 从纯崩溃表达，推进到“崩溃 -> 修复”这条更完整的情绪曲线。

适合的帖子角度：

- 通勤/工位修复仪式
- 低成本手作疗愈
- 失眠夜修补式生活记录
- 废物改造、旧物翻新、慢恢复

类型判断：`hybrid`

### 2. AI 效率 / 普通人用 AI / 知识种草

当前信号：

- 千瓜在 2025 年底的数据里提到，近 90 天小红书 AI 搜索浏览量超过 15 亿，知识类种草成为增长显著的内容形态。<https://www.qian-gua.com/information/detail/3225>
- 2026 年 Q1 热点解读里，`DeepSeek高能玩法`、`普通人用AI`、`职场撑伞位` 都被列进热点内容类型。<https://www.qian-gua.com/Home/ArticleDetail?id=3324>

为什么值得做：

- 这个垂类天然适合结构化研究、对比帖、模板帖和教程帖。
- 和 PTSM 当前 agent / harness 能力高度同频，最容易做出“研究即资产”的内容线。
- 它也是最容易通过评论区读到真实痛点的垂类之一。

适合的帖子角度：

- 一个具体工作流如何用 AI 提速
- 普通人可复制的 prompt 模板
- AI 工具对比与避坑
- 创作者/运营/职场人的低门槛自动化

类型判断：`trend + evergreen`

### 3. 轻养生 / 睡眠恢复 / 办公室健康

当前信号：

- 千瓜在 2026 年相关快报里把“健康生活新定义”列为小红书活跃方向，强调轻养生和日常恢复。<https://www.qian-gua.com/information/detail/3277>
- 同期观察中，`睡前洗漱` 相关话题浏览量约 4800 万，`睡前仪式感` 约 7400 万，说明“微恢复”内容有持续需求。<https://www.qian-gua.com/information/detail/3149>

为什么值得做：

- 内容门槛低，连续性强，适合做系列。
- 能和“发疯后的自救”“高压生活里的缓冲”自然衔接。
- 比纯医疗、纯健身更安全，也更贴近当前账号语气。

适合的帖子角度：

- 高压日常下的恢复流程
- 睡前 10 分钟的身体/情绪整理
- 办公室可做的小动作、小饮食、小习惯
- 咖啡、熬夜、失眠、恢复之间的日常平衡

类型判断：`evergreen`

### 4. 宠物陪伴 / 宠物户外

当前信号：

- 千瓜 2026 年 Q1 热词快报把“宠物和小孩一样”列为高热语义之一，同时提到宠物户外出游和亲子宠物概念在升温。<https://www.qian-gua.com/information/detail/3322>
- 相关行业观察里，`宠物户外装备` 已经成为明确消费与内容热点。<https://www.qian-gua.com/information/detail/3249>

为什么值得做：

- 宠物垂类互动稳定，评论区信息密度高。
- 很适合做真实经验帖、装备帖、路线帖和情绪陪伴帖。
- 如果后面要做多账号或多 playbook，这条线很容易独立出来。

适合的帖子角度：

- 宠物城市散步/近郊出行路线
- 宠物主人情绪陪伴日记
- 户外装备或照护用品清单
- 宠物友好空间、活动、节气玩法

类型判断：`hybrid`

### 5. 文博 / 非遗 / 地方文化体验

当前信号：

- 千瓜 2026 年 Q1 热词快报把“把春天织进非遗里”“文博游”“县城文学”等内容列进热点观察。<https://www.qian-gua.com/information/detail/3322>
- Q1 热点内容解读里也提到春日出游、地方体验和节气型内容的持续热度。<https://www.qian-gua.com/Home/ArticleDetail?id=3324>

为什么值得做：

- 适合 `classic_poetry_quote_post` 这类偏诗意、偏观察型表达，尤其是把经典诗词金句转成当代生活共鸣的短帖。
- 它比泛旅游更聚焦，也更容易做出稳定审美和账号辨识度。
- 可与手作、季节、地方生活方式互相串联。

适合的帖子角度：

- 一个城市里的小型文博体验
- 节气、花期、非遗工艺的观察帖
- 县城/街区/展览里的低强度漫游
- 器物、手艺、食物与地方情绪

类型判断：`evergreen with seasonal spikes`

## PTSM First Batch Recommendation

如果现在只选 2 到 3 条线先做，建议顺序更新为：

1. 轻养生 / 睡眠恢复 / 办公室恢复
2. 人类丰容 / 零成本日常变量实验
3. 修复系手作 / 情绪疗愈

原因：

- 这三条都能连续产出。
- 睡眠恢复和轻养生在 2026-05-30 live probe 中有最强常青互动信号，且能自然接入心理学安全边界、人类丰容低成本动作和发疯后的修复线。
- 人类丰容和修复系手作和当前 playbook 的语气距离最近，视觉证据、保存清单和评论交作业都清楚。
- 这三条最容易沉淀成可复用的 research skill，而不是只适合人工追热点。

`世界杯` 应作为 2026-05-28 权益新闻后的短期专项机会，由现有 `world_cup_daily_post` 承接，不计入常青 first batch。`AI 效率 / 普通人用 AI` 仍值得做，但本轮排在轻养生/睡眠恢复之后，原因是泛关键词 live score 中等，后续应靠更具体的工具/工作流关键词重新采样。

## How To Use This List

- 新选题进来时，先判断它是否属于以上 5 条之一。
- 如果不属于，不要立刻开新方向，先看它能否作为这 5 条的一个场景变体。
- 只有在连续 2 到 3 周都出现稳定信号时，才考虑新增垂类。
