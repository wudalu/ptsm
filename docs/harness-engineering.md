...
- XHS human-voice contract coverage through generic node constraints: `title_must_include_any` requires concrete title hooks for key playbooks, while `combined_must_not_include_any` scans title, cover text, and body together for template markers such as `首先`、`其次`、`综上`、`本文`、`作为AI` and other non-human/meta phrasing
...
- a deterministic, local-first cross-domain `guide-post` harness surface: domain tests cover the generic selector, including scene-keyword/lane-affinity separation, diversity-family selection, and the optional open-scene composer; application tests cover all current topic packs, larger-than-display candidate pools, scene-varying curated direction sets, and the `3 curated + 1 open_scene` hybrid contract; CLI tests cover JSON/Markdown output with `direction_type`; docs tests lock the generic and psychology OpenClaw wrapper contracts
