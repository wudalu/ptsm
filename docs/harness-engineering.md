...
- a modern psychology growth-sublane regression harness: unit tests now lock sleep recovery/light-wellness scenes to `睡眠恢复 / 轻养生`, `sleep_recovery_shutdown_card`, and semantic text-carousel guidance; e2e deterministic dry-run tests require a concise concrete title, 200-380 char body, 5-minute shutdown/save tool, role/camp comment prompt, professional-help boundary, and valid dynamic 1–18-page plan without medical wellness claims
- a psychology text-carousel harness: domain/drafting/runtime tests lock the exact closed parent fields plus slide-only `slide_id/order/role/headline/body_lines`, **one-topic one-set** dynamic 1–18 contiguous pages, emoji removal before validation, nested model normalization, safety scanning, and unchanged legacy/non-psychology plans. Ordinary memory tests lock cover-excluded canonical inner-page fingerprints, last-12-successful-receipt selection, concurrent reservation, failure/non-carousel release, and stale lease recovery; no marker may commit before a completely local-rendered canonical receipt plus page-aware ledger succeed, and learning-series stays isolated. The run boundary additionally rejects a missing or duck-typed reservation before renderer/ledger/publisher work, accepts only the exact capability bound to the current memory/namespace, and exercises receipt-intent recovery. Artifact-store regressions pin a previously missing root before workflow invoke and reject a workflow that mutates `base_dir`. Learning tests lock historic controlled-template-v1 single-card receipts, historic controlled-template-v2 carousel reconstruction, historic controlled-template-v3 warm editorial 7-page reconstruction, and current controlled-template-v4 semantic dynamic pagination with emoji-free image copy. Renderer/transaction tests cover 1080×1440 role variants, visible `NN / TT` page counters, content-addressed manifests, page/file hashes, path containment, idempotent reuse and atomic complete-set visibility. Run/ledger/publisher tests require manifest order, set-wide descriptor/snapshot verification, lock-serialized page-aware ledger projection under concurrent appends, and descriptor-pinned `base_dir` plus fixed `outputs/artifacts/generated-image-assets` ancestors opened/created one component at a time with `dir_fd` + no-follow. Regressions rebind or replace intermediate ancestors around locking, replace and directory fsync and require fail-closed behavior; the same suite requires local watermark skip, no `carousel_delivery.status=ready`, and zero publisher/relay side effects on page, manifest or ledger failure, with stable status `psychology_carousel_generation_failed`. Ready ordinary receipts must contain exact ordered `attachments` with matching `slide_id` and canonical per-page `page_sha256` / `file_sha256`; wrapper/docs tests require one ordered multi-image sender call when supported, otherwise serial `attachments[].order` transmission with an ACK before the next call, and forbid path/filename sorting. PTSM tests no external chat/IM sender, sender ACK/outcome/retry persistence, or delivery claim. Requested-image learning tests advance production progress only after the complete set, page-aware operational ledger and strict artifact pass, while no-image tests retain the prior content-artifact timing.
- renderer layout regressions lock defense-in-depth emoji removal at the psychology image boundary, a shared horizontal center line for headline/body copy, and a vertically balanced whole-copy block after real wrapping; maximum legal copy must remain inside the card safe area.
- guide/docs/wrapper tests lock ordinary `format_archetype=text_carousel`, local `psychology_text_card_v1`, a dynamic 1–18 page range and ordered semantic roles without introducing a manual pagination or batch flag. They require content beyond 18 pages to stop for shortening or separately confirmed `multiple_posts` rather than split/loop/repeat it. Docs tests also lock the relay-owned ACK/outcome/retry boundary: local `ready` is not delivered, and only a relay-confirmed full ordered set may be called delivered. Current learning directions expose the exact dynamic page count and repeated ordered roles returned by controlled-template-v4 while historic controlled-template-v1 directions remain `note_card`; the wrapper may display only PTSM-returned `page_count` / `ordered_roles` structure before the run, must not claim `guide-post` returned page copy, and may not author page text or add an image override.
