...
- a psychology learning-series harness: domain tests lock one closed, versioned six-lesson catalog, distinct catalog title/cover hooks, and reject any extra reader-visible assertion beyond the controlled template; CLI/guide tests prove that only `modern_psychology_post` can choose `catalog_learning_series` directions, that an unselected roadmap returns `selection_required` rather than silently choosing lesson one, and that a run pins curriculum version with no manual image override; application preflight rejects missing, malformed or cross-lesson selection before a run exists; and runtime-boundary tests prove all six lessons complete while raw scene/source values never reach state, checkpoints, prompt, memory or artifact—even if an ordinary psychology run reused the public thread id first. Runtime construction re-resolves and exactly compares the catalog contract/manifest. The same exact lesson contract gates deterministic/model drafts at executor, reflector, finalize and pre-publish application layers; free-scene reflection/judge rules cannot retry a catalog-valid lesson. `psychology.learning_receipt` reconstructs the expected catalog lesson from the artifact's safe identity and scans the entire artifact for raw provenance, failing a missing, tampered or cross-lesson manifest/gate without echoing raw values. Owned unsafe custom artifacts are removed before publish. Metrics tests lock receipt-verified series/version/lesson grouping, same artifact/checkpoint upsert and the existing `needs_more_data` threshold.
...
- a modern psychology growth-sublane regression harness: unit tests now lock sleep recovery/light-wellness scenes to `睡眠恢复 / 轻养生`, `sleep_recovery_shutdown_card`, and `iphone_notes` save-tool guidance; e2e deterministic dry-run tests require a concise concrete title, 200-380 char body, 5-minute shutdown/save tool, role/camp comment prompt, and professional-help boundary without medical wellness claims
