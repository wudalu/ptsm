...
- a psychology text-carousel harness: domain/drafting/runtime tests lock the exact closed parent fields plus slide-only `slide_id/order/role/headline/body_lines`, **one-topic one-set** 4–7 contiguous pages, nested model normalization, safety scanning, and unchanged legacy/non-psychology plans. Ordinary memory tests lock cover-excluded canonical inner-page fingerprints, last-12-successful-receipt selection, concurrent reservation, failure/non-carousel release, and stale lease recovery; no marker may commit before a completely local-rendered canonical receipt plus page-aware ledger succeed, and learning-series stays isolated. The run boundary additionally rejects a missing or duck-typed reservation before renderer/ledger/publisher work, accepts only the exact capability bound to the current memory/namespace, and exercises receipt-intent recovery. Artifact-store regressions pin a previously missing root before workflow invoke and reject a workflow that mutates `base_dir`. Learning tests lock historic controlled-template-v1 single-card receipts, historic controlled-template-v2 carousel reconstruction, and current controlled-template-v3 warm editorial 7-page reconstruction with emoji-free image copy. Renderer/transaction tests cover 1080×1440 role variants, content-addressed manifests, page/file hashes, path containment, idempotent reuse and atomic complete-set visibility. Run/ledger/publisher tests require manifest order, set-wide descriptor/snapshot verification, lock-serialized page-aware ledger projection under concurrent appends, and descriptor-pinned `base_dir` plus fixed `outputs/artifacts/generated-image-assets` ancestors opened/created one component at a time with `dir_fd` + no-follow. Regressions rebind or replace intermediate ancestors around locking, replace and directory fsync and require fail-closed behavior; the same suite requires local watermark skip, no `carousel_delivery.status=ready`, and zero publisher/relay side effects on page, manifest or ledger failure, with stable status `psychology_carousel_generation_failed`. Ready ordinary receipts must contain exact ordered `attachments` and canonical per-page `page_sha256` / `file_sha256`; wrapper/docs tests require relay transmission in strict `attachments[].order` order and forbid path/filename sorting. PTSM tests no external chat/IM sender, sender ACK/outcome/retry persistence, or delivery claim. Requested-image learning tests advance production progress only after the complete set, page-aware operational ledger and strict artifact pass, while no-image tests retain the prior content-artifact timing.
- guide/docs/wrapper tests lock ordinary `format_archetype=text_carousel`, local `psychology_text_card_v1`, one 4–7 page range and ordered semantic roles without introducing a manual pagination or batch flag. They require a >7/12 request to choose supported `one_carousel`, separately confirmed `multiple_posts`, or unsupported `independent_assets` rather than split/loop/repeat/promote it. Docs tests also lock the relay-owned ACK/outcome/retry boundary: local `ready` is not delivered, and only a relay-confirmed full ordered set may be called delivered. Current learning directions expose the exact 7-role carousel format while historic controlled-template-v1 directions remain `note_card`; the wrapper may display only PTSM-returned `page_count` / `ordered_roles` structure before the run, must not claim `guide-post` returned page copy, and may not author page text or add an image override.
