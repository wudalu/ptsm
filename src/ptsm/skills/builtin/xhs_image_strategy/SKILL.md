...
- `carousel_style` / `slides`: 只用于现代心理学自动轮播。固定 `carousel_style=psychology_text_card_v1`，并在同一次写作中给出 4-7 张有序 `slides`；每页只能有 `slide_id`、`order`、`role`、`headline`、`body_lines`。`role` 必须用精确枚举值，按语义顺序逐页对应：封面页 `cover_hook`、具体场景页 `concrete_scene`、轻机制页 `light_mechanism`、可保存工具页 `save_tool`、范围边界页 `scope_boundary`、专业边界页 `professional_boundary`、评论入口页 `comment_prompt`；不得自造 `scene`、`mechanism`、`boundary` 这类自然语言值。
...
- 现代心理学文字轮播：默认用本地 `psychology_text_card_v1`，`backend=local_social_screenshot`、`style=psychology_text_card`、`role=text_carousel`。用 4-7 张卡只讲一个主题，语义顺序为封面钩子、具体场景、轻机制、可保存工具、范围/专业边界、评论入口；可以按主题合并边界页，但不能按正文长度机械分页，也不能再调用模型改写页文案。逐页 `role` 精确取值：封面钩子 `cover_hook`、具体场景 `concrete_scene`、轻机制 `light_mechanism`、可保存工具 `save_tool`、范围边界 `scope_boundary`、专业边界 `professional_boundary`、评论入口 `comment_prompt`。
